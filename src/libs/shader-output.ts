...
export const generateShaderOutput = (data: any) => {
  ...
  const matNameType: string = matName
    .split('_')
    .slice(-1)[0]
    .toLocaleLowerCase();
  const isFaceMat = matNameType === 'face';
  ...
  return `// ${matName} (generated by TurN)
...
${isFaceMat ? '' : '// '}#define USE_FACE_SHADOW_MAP
